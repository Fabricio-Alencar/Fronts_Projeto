/* ============================================================
   ...
   (() => {
    ...
    const API_BASE = "https://fabricioapis.azurewebsites.net"; // URL base da API
    ...
    const editStatus = document.getElementById('editStatus'); // Novo campo para o status
    ...
    const status = document.querySelector('.status .valor'); // Elemento de status
    ...
    async function abrirModal() {
      ...
      try {
        ...
        editStatus.value = projeto.status || "Em andamento"; // Preenche com o status atual do projeto
    ...
    if (btnSalvar) btnSalvar.addEventListener('click', async () => {
      try {
        ...
        status.textContent = projetoAtualizado.status; // Atualiza o status na tela
  
        // Atualiza tecnologias e repositórios no backend
        await atualizarItens(editTecnologias, 'tecnologias');
        await atualizarItens(editRepositorios, 'repositorios');
  
        // ⚡️ Atualiza listas globais de tecnologias e repositórios (se existirem)
